...
{
    public partial class andalAge : Form
    {
        public andalAge()
        {
            InitializeComponent();
        ...
        private void searchButton_Click(object sender, EventArgs e)
        {
            ...
            string Andal_AgeInput;//creates string for year

            Andal_AgeInput = andalYear.Text;// makes user input

            while (AndalAge_Switch == false)
            {
                for (j = 0; j < AndalAge_Array.Length; j++)//iterates through the array
                {
                    if (j == AndalAge_Array.Length - 1)//sets iteration switch when iteration reaches the end of the array
                    {
                        Iteration_Switch = true;
                    }

                    if (Andal_AgeInput == AndalAge_Array[j])//outputs information if user input is found within the array
                    {
                        MessageBox.Show(AndalAge_Array[j + 1]);
                        AndalAge_Switch = true;
                        break;
                    }
                    else if (Iteration_Switch == true && Andal_AgeInput != AndalAge_Array[j])//handles user input if it is not found within the array
                    {
